 ...
 */
export async function createWorldServer(config, hooks) {
    const { worldId, worldName = `World (${worldId})`, worldTheme = "default", port = 8099, publicPort, publicAddr = null, dataDir = "/data", bootstrapUrl = DEFAULT_BOOTSTRAP_URL, maxAgents = 0, isPublic = true, password = "", broadcastIntervalMs = 5_000, discoveryIntervalMs = 10 * 60 * 1000, staleTtlMs = 30 * 60 * 1000, setupRoutes, } = config;
    ...
    async function broadcastWorldState() {
        ...
        const snapshot = { worldId, worldName, theme: worldTheme, ...(state ?? {}) };
        ...
        await Promise.allSettled(knownPeers.map(async (peer) => {
    ...
    const pruneTimer = setInterval(() => {
        ...
        if (pruned > 0)
            console.log(`[world] Pruned ${pruned} stale peer(s)`);
    }, 5 * 60 * 1000);
    // Bootstrap discovery
    let stopDiscovery;
    if (isPublic) {
